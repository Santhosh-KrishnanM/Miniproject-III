...
const TRAVEL_VEHICLES = [
  { id: 'tempo', name: 'Tempo Traveller', seats: 17, cost: 600, img: 'tempo.jpg' },
  { id: 'thar', name: 'Thar', seats: 6, cost: 450, img: 'thar.jpg' },
  { id: 'travels', name: 'Travels Bus', seats: 60, cost: 1000, img: 'travels.jpg' }
];
...
async function openTravelPopup(vehicle) {
  ...
  const filtered = bookings.filter(b => b.destination);
  if (filtered.length === 0) {
    select.innerHTML += `<option value="" disabled>No active bookings found</option>`;
  } else {
    filtered.forEach(b => {
      const start = b.startDate ? b.startDate.slice(0,10) : '';
      const end = b.endDate ? b.endDate.slice(0,10) : '';
      const title = `${b.destination?.name || 'Booking'} — ${start} → ${end}`;
      select.innerHTML += `<option value="${b._id}" data-start="${b.startDate}" data-end="${b.endDate}">${title}</option>`;
    });
  }
...
async function confirmTravelBooking() {
  ...
  if (!bookingId || !vehicle) {
    alert("Please select a booking before confirming travel.");
    return;
  }
  ...
  try {
    const res = await fetch(`/api/bookings/${bookingId}`);
    if (!res.ok) {
      alert("Failed to load booking details from server.");
      return;
    }
  ...
  if (!currentBooking.userId && currentUser?._id) {
    currentBooking.userId = currentUser._id; // ✅ add userId if missing
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      alert("Failed to save travel data: " + msg);
      return;
    }

    closeTravelPopup();
    showSuccessToast(`✅ ${vehicle.name} booked successfully!`);
    ...
    alert("Error saving travel booking. Check console for details.");
  }
}
